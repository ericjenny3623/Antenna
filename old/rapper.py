...
if __name__ == "__main__":
    ...
    for ensemble in range(0, settings.ENSEMBLES):
        ...
        if ensemble == 0:
            with open(filepath + "x.txt", "w") as xfile:
                writer = csv.writer(xfile)
                writer.writerows(x)
            with open(filepath + "R.txt", "w") as Rfile:
                writer = csv.writer(Rfile)
                writer.writerows(R)
            with open(filepath + "fitness.txt", "w") as fitnessfile:
                writer = csv.writer(fitnessfile)
                writer.writerow(fitness)
        else:
            with open(filepath + "x.txt", "a") as xfile:
                writer = csv.writer(xfile)
                writer.writerows(x)
            with open(filepath + "R.txt", "a") as Rfile:
                writer = csv.writer(Rfile)
                writer.writerows(R)
            with open(filepath + "fitness.txt", "a") as fitnessfile:
                writer = csv.writer(fitnessfile)
                writer.writerow(fitness)
